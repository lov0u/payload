...
function stripTitleJunk(t: string): string {
  return (t || '')
    ...
    .replace(/^\s*\d+[\.、)]\s*/, '')
    .replace(/[“"”'‘’「」『』【】]/g, '')
    // 模型元指令泄漏：「用户需要一个SEO标题，关键词是…」「以下是…」「好的，…」
    .replace(/^(?:好的|当然|明白了)[，,、]?\s*/g, '')
    .replace(/(?:用户(?:需要|想要|要求)|以下是|这里是|我(?:来|会|将))[^，。\n]*/g, '')
    .replace(/[，,、：:]\s*关键词是.*$/g, '')
    // 勾选/装饰符号 ✓ ✔ √ ★ ☆ 等
    .replace(/[✓✔√✗✘×★☆♦◆→←↑↓]/g, '')
    .replace(/[（(][^（）()]*\d+\s*字[^（）()]*[）)]/g, '')
...
function isTitleMalformed(title: string, keyword: string): boolean {
  ...
  // 模型把提示词/思考过程当成标题输出
  if (/(用户|SEO标题|关键词是|以下是|这里是|好的|请注意|标题[：:]|输出)/.test(title)) return true
  // 残留 markdown / 装饰符号
  if (/[*#`✓✔√✗✘★☆|]/.test(title)) return true
  // 标题过长（正常应 ≤14 汉字，超 20 基本是把说明文字带进来了）
  if (title.replace(/[^\u4e00-\u9fa5]/g, '').length > 18) return true
  return false
}
...
【图表（必做，SEO 安全写法 + 量纲一致 + 数据完整）】
- **每篇文章必须包含 1 个可视化表格**（有充分素材时可放 2 个，不要超过 2 个）。这是硬性要求。
- 写法：语义化 <table>（含 <thead><tbody>），并给 <table> 加 data-chart="bar" 或 data-chart="pie"，以及 data-chart-title="图表标题"。
- 选型：bar 用于跨项横向对比（不同方案/车型/材质/服务的成本、时效、评分等）；pie 用于构成占比（费用构成、市场份额、成分配比）。
- **⚠️ 同一张 bar 表的所有数值列必须使用同一量纲**（例如全部 1-5 分、全部 0-100%、全部倍数 0.5-2.0）。如果一列是「评分 1-5」而另一列是「价格区间」，它们的数字量级差异会导致小柱被压扁看不见——遇到这种情形，要么把不可比的列改成纯文字描述（不放进表头当 series），要么拆成两张表。
- **⚠️ 纯文字列（如「推荐险种」「保障重点」「适合场景」「险种名称」等）不要放进表头当 series**——它们无法被解析为数字，会让图例出现却画不出柱。只放真正可比的数据列。
- **⚠️ pie 表每一行的数值都必须填实数**（不能留空、用「—」「暂无」「-」代替），否则饼图会塌缩成一个圆。所有行的占比合计应为 100。
- 数值单元格只放纯数字（如 1200、35、4.5），单位写进表头或 data-chart-title；区间数据（如 3000-12000）也可以写，系统会自动取上限解析。
- **怎么做到"必有图表"又不编造数据**：不要求你给出精确的官方统计。可以用下面这些"可靠且可比"的角度建表——
  · 相对系数/倍数（以最常见方案为 1.0 基准，其它按常识给相对值）
  · 评分/等级量化（如耐用性、性价比、上手难度，按 1-5 分给出你的专业判断）
  · 常识性区间（行业内公认的大致范围，写成区间即可）
  · 构成占比（各项在总成本/总时间里的大致占比，合计 100）
  用这类"经验判断型"数据时，在表格前后用一句话说明这是经验参考值、实际以具体情况为准，不要包装成权威统计。
- 仍然禁止：编造具体机构名、研究报告、精确到小数的伪统计（如"据XX协会数据，占比 37.2%"）。
...
再次强调：文章中必须包含 1 个带 data-chart 属性的对比表格（bar 或 pie），这是必须项，不能省略。`,
      },
    ]
